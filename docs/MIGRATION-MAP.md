...
| Techgenyz | post views | public AJAX `tgm_count_post_view`; exact legacy meta key to be re-verified | `modules/analytics/` | SHARED CORE CAPABILITY + TECHGENYZ LEGACY COMPATIBILITY | Preserve lifetime baseline/cutover; replace postmeta writes; retain action while needed; prevent duplicates; dedicated storage, privacy-safe daily estimate, bot/staff exclusion and realtime/top/trending tests | 0.1.0 |
